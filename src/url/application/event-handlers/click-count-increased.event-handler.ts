...
export class ClickCountIncreasedHandler implements IEventHandler<ClickCountIncreasedEvent> {
  private readonly logger = new Logger(ClickCountIncreasedHandler.name);
  constructor(private readonly urlRepository: UrlRepository) {}

  async handle(event: ClickCountIncreasedEvent) {
    this.logger.debug(`[Event Handler] Click count increased for URL: ${event.url.id}`);

    await this.urlRepository.incrementClickCount(event.url.id);
    this.logger.debug(`[Event Handler] Click count incremented for URL: ${event.url.id}`);
  }
}
